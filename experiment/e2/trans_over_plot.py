...
def plot_with_legend(all_data, color_dict, hatch_dict):
    ...
    new_labels = ['Multi-hop' if 'Multi-hop Priority' in label.get_text() else 'Priority' if 'Priority' in label.get_text() else 'Multi-hop' if 'Multi-hop Normal' in label.get_text() else 'Normal' for label in ax.get_yticklabels()]
    ax.set_yticklabels(new_labels)

    # Create custom legend
    legend_elements = [
        plt.Line2D([0], [0], color=sns.color_palette()[3], lw=4, label='Star Topology'),
        plt.Line2D([0], [0], color=sns.color_palette()[0], lw=4, label='Multi-hop Topology'),
    ]
...
configurations2 = ['Multi-hop topology']
...
color_dict = {
    ...
    'Multi-hop topology Priority': sns.color_palette()[0],
    'Multi-hop topology Multi-hop Priority': sns.color_palette()[0],
    'Star topology Normal': sns.color_palette()[3],
    'Multi-hop topology Normal': sns.color_palette()[0],
    'Multi-hop topology Multi-hop Normal': sns.color_palette()[0],
}
