...
overhead_throughput_data = pd.read_csv('sen_avg_overhead_throughput_topic_priority.csv')
...
for _, row in pareto_data_3d.iterrows():
    ax.text(row['Average Throughput'], row['avg_lag']+0.18, row['Average Throughput_overhead'],
             dummy_text_data_3d[row['File']], fontsize=12, ha='left', va='top', color='k', fontweight='bold', zorder=10, bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
...
# Save figure
plt.savefig(f'../e5/pareto3d_priority.png', bbox_inches='tight', pad_inches=0.05, dpi=300)